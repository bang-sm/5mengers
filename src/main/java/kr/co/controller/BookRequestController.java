...
public class BookRequestController {
	...
	private static final Logger logger = LoggerFactory.getLogger(BookRequestController.class);
		...
		public String list(Model model,Criteria cri, PageMaker pageMaker, BookRequestDTO bRequestDTO) throws Exception {
			logger.info("brb/list");
			// 구매 요청 게시물 총 갯수
			pageMaker.setCri(cri);
			int brbcount=service.brbListCount();
			pageMaker.setTotalCount(brbcount);
			int brb_id = bRequestDTO.getBrb_id();
			BookRequestDTO category = service.category(brb_id);
		...
		public String readView(BookRequestDTO bRequestDTO, Model model,HttpSession hs,UserVO uv) throws Exception {
			...
			int uuid = uv.getUuid();
			System.out.println(bRequestDTO.getBrb_id());
			logger.info("여기");
			model.addAttribute("read", service.read(bRequestDTO.getBrb_id()));
			model.addAttribute("user", uuid);
			model.addAttribute("replyRead",service.replyRead(bRequestDTO.getBrb_id()));
		...
		// 댓글 작성하기
		@RequestMapping(value = "brb/replyWrite", method = RequestMethod.POST)
		public String replyWrite(BookRequestDTO bRequestDTO) throws Exception {
			logger.info("brb/replyWrite");
			
			service.replyWrite(bRequestDTO);
			
			return "redirect:/brb/list";
		}
		
		// 댓글 삭제하기
		@RequestMapping(value = "brb/replyDelete", method = RequestMethod.GET)
		public String replyDelete(int brbr_id) throws Exception {
			logger.info("/brb/replyDelete");
			
			service.replyDelete(brbr_id);
			return "redirect:/brb/list";
		}
